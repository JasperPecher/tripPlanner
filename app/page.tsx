...
import { Compass, Users, Receipt, Camera, Sun, Moon, Globe } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-gradient-to-br from-orange-500 to-amber-600 dark:from-orange-800 dark:to-stone-900 text-white">
        <div className="max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <div className="flex justify-end gap-2 mb-6">
            <button onClick={() => setLocale(locale === "de" ? "en" : "de")} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-sm transition">
              <Globe className="w-4 h-4" />
              {locale === "de" ? "EN" : "DE"}
            </button>
            <button onClick={toggleTheme} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg transition">
              {theme === "dark" ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
          </div>
          <div className="text-center">
            <Compass className="w-16 h-16 mx-auto mb-6" />
            <h1 className="text-4xl sm:text-5xl font-bold mb-4">{t.home.title}</h1>
            <p className="text-xl text-orange-100 dark:text-orange-200 max-w-2xl mx-auto">{t.home.subtitle}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-lg p-8 max-w-xl mx-auto border border-stone-200 dark:border-stone-800">
          <h2 className="text-2xl font-bold text-stone-900 dark:text-white mb-6 text-center">{t.home.createTitle}</h2>
          <CreateTripForm />
        </div>
      </div>
    </div>
