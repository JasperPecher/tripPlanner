...
export function CreateTripForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label htmlFor="adminName" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
          {t.home.adminName}
        </label>
        <input id="adminName" type="text" required value={formData.adminName}
          onChange={(e) => setFormData({ ...formData, adminName: e.target.value })}
          className={inputClasses} placeholder={t.home.adminNamePlaceholder} />
      </div>
      ...
      <div>
        ...
        <input id="name" type="text" required value={formData.name}
          ...
          className={inputClasses} placeholder={t.home.tripNamePlaceholder} />
      </div>
      ...
      <div>
        ...
        <textarea id="description" value={formData.description}
          ...
          className={inputClasses + " resize-none"} rows={2} placeholder={t.home.descriptionPlaceholder} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="startDate" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
            {t.home.startDate}
          </label>
        ...
        <div>
          <label htmlFor="endDate" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
            {t.home.endDate}
          </label>
      ...
      <button type="submit" disabled={loading}
        ...
        {loading ? (<><Loader2 className="w-5 h-5 animate-spin" />{t.home.creating}</>) : (t.home.createButton)}
      </button>
    </form>
  );
}
